...
Function.prototype.bindArgs = function () {
    var bindedArgs = Array.from(arguments),
        that = this;
    //console.log(bindedArgs, that);
    return function () {
        var args = bindedArgs.concat(Array.from(arguments));
        that.apply(null, args);
    }
};
Function.prototype.bindArgsAfter = function () {
    var bindedArgs = Array.from(arguments),
        that = this;
    //console.log(bindedArgs, that);
    return function () {
        var args = Array.from(arguments).concat(bindedArgs);
        that.apply(null, args);
    }
};
...
var Task = {
	getFileArray: function(html) {
        ...
        var counter = (this !== window && this.counter) ? (this.counter) : {id: 0, get:() => counter.id++},
    ...
    openDownloader: function() {
        ...
        if(Data.contentArray.length === 0) {
            ...
            Promise.resolve(contentUrlArray)
                .map((contentUrl, index) => Promise.resolve(contentUrl.url)
                    .then(Http.downloadPage)
                    .then(Task.getFileArray.bind({counter: counter, parentId: index}))
                    .then((subContentArray) => 
                        [{
                          ...
                          .concat(subContentArray)
                    )
                )
				.reduce((prev, next) => prev.concat(next)) // 2D -> 1D
